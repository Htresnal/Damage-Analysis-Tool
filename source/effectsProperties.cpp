...
void effectsProperties::OnButton2Click(wxCommandEvent& event)
{
    ...
    if (answer == wxOK)
    {
        ...
        if (i!=-1)
        {
            ...
            {
                if (DamageInputWindow->isSameRefAddress((*(basic_effect *)DamageInputWindow->attackEffectsCtrl->GetItemData(i)),**tempVecIt)==true)
                {
                    basic_effect *myPtr=&(*(basic_effect *)DamageInputWindow->attackEffectsCtrl->GetItemData(i));
                    delete (basic_effect *)DamageInputWindow->attackEffectsCtrl->GetItemData(i);
                    attackEffects.erase(tempVecIt);
                    DamageInputWindow->attackEffectsCtrl->DeleteItem(i);
                }
            }
        }
        std::vector<basic_effect *>::iterator tempVecIt2=defenceEffects.end()-1;
        i=DamageInputWindow->defenseEffectsCtrl->FindItem(-1,(wxUIntPtr)ptrtoItem);
        if (i!=-1)
        {
            int vectSize2=defenceEffects.size()-1;
            for (int j=defenceEffects.size()-1;vectSize2==defenceEffects.size()-1;j--,tempVecIt2--)
            {
                if (DamageInputWindow->isSameRefAddress((*(basic_effect *)DamageInputWindow->defenseEffectsCtrl->GetItemData(i)),**tempVecIt2)==true)
                {
                    basic_effect *myPtr=&(*(basic_effect *)DamageInputWindow->defenseEffectsCtrl->GetItemData(i));
                    delete (basic_effect *)DamageInputWindow->defenseEffectsCtrl->GetItemData(i);
                    defenceEffects.erase(tempVecIt2);
                    DamageInputWindow->defenseEffectsCtrl->DeleteItem(i);
                }
            }
        }
